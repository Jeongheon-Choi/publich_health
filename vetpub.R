...
## 영양제의 치료효과를 확인함
## id : 방문견 번호
## group: low- 저용양  high- 고용량  control- 먹이지 않음
## gender : 성별,  F=Female/암컷  M=Male/수컷 
## age : 나이, 단위는 년
## BW : 몸무게  body weight,  단위는 kg
## BW_before : 프리바이오틱스를 먹기전 몸무게
## BW_after : 프리바이오틱스를 먹은후 몸무게
## Blautia_before : 프리바이오틱스를 먹기전 blautia 균의 양
## Blautia_after : 프리바이오틱스를 먹은후 blautia 균의 양
...
mean(dog$BW_before) # 프리바이오틱스를 먹기전 몸무게 평균 mean
sd(dog$BW_before) # 프리바이오틱스를 먹기전 몸무게 표준편차, standard deviation

mean(dog$BW_after) # 프리바이오틱스를 먹은후 몸무게 평균 mean
sd(dog$BW_after) # 프리바이오틱스를 먹은후 몸무게 표준편차, standard deviation
...
ggplot(dog)+
  geom_point(aes(x=group, y=Blautia_before))

# 사이즈 3, 색깔 파란색, 모양 X자
ggplot(dog)+
  geom_point(aes(x=group, y=Blautia_before),size=3,color="blue",shape=4)
...
# 고용량군 추출
high <- subset(dog,subset = group == 'high')
high
t.test(high$Blautia_before, high$Blautia_after, paired = TRUE)
